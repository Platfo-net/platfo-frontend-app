  ...
  <div class="flex row">
    <template v-if="shopCart.getItemCount(product) > 0">
      <div class="flex row q-pa-sm justify-between items-center"
        ...
        <q-btn @click="shopCart.add(product)" class="q-mx-sm" icon="add" color="white" text-color="grey-8"
          size="sm" dense flat></q-btn>
        <small class="q-mx-sm">{{ shopCart.getItemCount(product) }}</small>
        <q-btn @click="shopCart.remove(product)" class="q-mx-sm"
          :icon="shopCart.getItemCount(product) === 1 ? 'delete' : 'remove'"
          :text-color="shopCart.getItemCount(product) === 1 ? 'red-8' : 'dark'" size="sm" dense flat></q-btn>
      </div>
    </template>
    <template v-else>
      <div class="flex row">
        <q-btn color="dark" label="افزودن" @click="shopCart.add(product)" style="width: 143px;"></q-btn>
      </div>
    </template>
  </div>
</template>
