        ...
        <div class="flex row">
          <template v-if="shoppingCart.getItemCount(product) > 0">
            <div class="flex row q-pa-sm justify-center items-center" style="border: 1px solid #e2e2e2; border-radius: 4px; box-shadow: #e2e2e2 0px 0px 15px;">
              ...
              <q-btn @click="shoppingCart.remove(product)" class="q-mx-sm" :icon="shoppingCart.getItemCount(product) === 1 ? 'delete' : 'remove'" :text-color="shoppingCart.getItemCount(product) === 1 ? 'red-8' : 'dark'" size="sm" dense flat></q-btn>
            </div>
          </template>
          <template v-else>
            <div class="flex row">
