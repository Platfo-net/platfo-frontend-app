  ...
  <q-scroll-area style="height: calc(100% - 161.6px); margin-top: 161.6px">
    <q-list padding>
      <div v-for="(route, idx) in routes" :key="route.title + idx">
        <q-item
          :active-class="
            route.activeClass?.length ? route.activeClass : 'bg-teal-1'
          "
          clickable
        ...
        <q-expansion-item v-else :label="route.title" :icon="route.icon">
          <div
            v-for="(innerRoute, innerIdx) in route.children"
            :key="innerRoute.title + innerIdx"
          >
            <q-item
              :active-class="
                innerRoute.activeClass?.length
                  ? innerRoute.activeClass
                  : 'bg-teal-1'
              "
              v-if="!innerRoute.children?.length"
              :key="innerRoute.title + innerIdx"
              class="q-pl-xl"
              clickable
              :to="innerRoute.to"
            ...
            >
              <q-item
                ...
                ) in innerRoute.children"
                :key="innerInnerRoute.title + innerInnerRouteIdx"
                class="pl-2xl"
                clickable
