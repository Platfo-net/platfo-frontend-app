...
const enableCheckout = computed(() => dataToSend.shipment_method_id?.length || !dataToSend.payment_method_id?.length);
...
<template>
  <q-dialog v-model="showCheckoutDialog" persistent maximized transition-show="slide-up" transition-hide="slide-down">
    <q-card class="bg-white">
      ...
      <q-card-section>
        ...
        <q-input class="q-my-md" dense outlined color="orange" v-model="dataToSend.first_name" name="first_name"
          label="نام"></q-input>
        <q-input class="q-my-md" dense outlined color="orange" v-model="dataToSend.last_name" name="last_name"
          label="نام خانوادگی"></q-input>
        <q-input dir="ltr" class="q-my-md" dense outlined color="orange" v-model="dataToSend.phone_number"
          name="phone_number" label="شماره تماس"></q-input>
        <q-input class="q-my-md" dense outlined type="textarea" color="orange" v-model="dataToSend.address" name="address"
          label="آدرس محل سکونت"></q-input>
      </q-card-section>
      <q-card-section>
        ...
        <template v-else>
          <q-list>
            <q-item v-ripple class="q-my-md rounded-borders" style="border: 1px solid #e1e1e1"
              v-for="shippingMethod in shopShippingMethods" :key="shippingMethod.id" tag="label">
              <q-item-section avatar>
                <q-radio v-model="dataToSend.shipment_method_id" :val="shippingMethod.id" color="orange" />
              </q-item-section>
              <q-item-section>
                <q-item-label>{{ shippingMethod.title }}</q-item-label>
                <q-item-label caption>هزینه ارسال:
      ...
      <q-card-section>
        ...
        <template v-for="paymentMethod in shopPaymentMethods" :key="paymentMethod.id">
          <template v-if="paymentMethod.items.length">
            <template v-for="innerPm in paymentMethod.items" :key="innerPm.id">
              <div class="q-ml-lg q-my-md">پراخت آنلاین</div>
              <q-list>
                <q-item class="q-my-md rounded-borders" style="border: 1px solid #e1e1e1" tag="label">
                  <q-item-section avatar>
                    <q-radio v-model="dataToSend.payment_method_id" :val="innerPm.id" color="orange" />
                  </q-item-section>
                  <q-item-section>
      ...
      <q-card-section>
        <div class="flex row q-pa-md q-px-lg items-center justify-between blur">
          <div class="flex column text-dark">
            ...
            <div>
              {{
                Intl.NumberFormat('fa', {
                  currency: 'IRT',
                }).format(shoppingCart.totalCartAmount() + (shopShippingMethods?.find(x => x.id ===
                  dataToSend.shipment_method_id)?.price as number))
              }}
              <toman-symbol :size="16"></toman-symbol>
            </div>
          </div>
          <!-- <q-btn @click="showCheckoutDialog = true" label="مرحله&zwnj;ی بعد" color="orange" /> -->
          <q-btn @click="showConfirmDialog = true" color="orange" :disable="!enableCheckout">پرداخت
            سفارش</q-btn>
        </div>
      </q-card-section>
    </q-card>
  ...
  <q-dialog class="font-vazir" v-model="showConfirmDialog">
    <q-card>
      ...
      <q-card-actions>
        <q-btn size="sm" color="primary" :loading="loading" @click="checkout">بله</q-btn>
        <q-btn size="sm" color="negative" :disable="loading" @click="showConfirmDialog = false">خیر</q-btn>
      </q-card-actions>
    </q-card>
  </q-dialog>
  ...
  <div class="flex row q-pa-md q-px-lg items-center justify-between blur" style="border-bottom: 1px solid #e2e2e2">
    <div class="flex column text-dark">
