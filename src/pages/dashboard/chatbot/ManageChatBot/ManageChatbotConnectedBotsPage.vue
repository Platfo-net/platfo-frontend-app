...
const creditSections: CreditSectionTabLink[] = [
    ...
    { id: 'connected_instagram_bots', title: 'بات های اینستگرام', icon: 'instagram' },
]
/******************** */
        ...
        <q-card flat bordered style="min-width: 275px;">
            ...
            <q-card-section>
                <q-select lazy-rules class="q-my-md"
                    :options="telegramBots?.map((x) => ({ label: x.first_name, value: x.id }))" outlined label="بات"
            ...
            <q-header bordered class="bg-white text-primary" height-hint="98">
                <q-toolbar>
                    <div class="flex row justify-between items-center full-width">
                        <q-btn flat :icon="showBotsDrawer ? 'chevron_right' : 'chevron_left'"
                            :label="showBotsDrawer ? 'بستن منو' : 'منو'" @click="showBotsDrawer = !showBotsDrawer" />
                        <div class="text-body1">بات های متصل</div>
                    </div>
                    <!-- <q-toolbar-title>مدیریت اعتبار</q-toolbar-title> -->
                </q-toolbar>
            </q-header>
            <q-drawer class="flex column justify-between q-pa-md overflow-auto" show-if-above v-model="showBotsDrawer"
                side="left" bordered>
                <q-list dense>
                ...
                <q-page class="q-pa-md">
                    <q-tab-panels v-model="manageCreditSection">
                        <q-tab-panel name="connected_telegram_bots">
                            ...
                            <p>در این بخش، شما میتوانید بات های تلگرامی خود را که در بخش <router-link
                                    :to="{ name: 'BotListPage' }">پلتفرم
                                    ها</router-link>
                                اضافه کردید را به این چت بات متصل کنید.</p>
                            ...
                            <div v-else>
                                <div class="row q-col-gutter-md">
                                    <div v-for="bot in connectedTelegramBots" class="col-12 col-md-4" :key="bot.uuid">
