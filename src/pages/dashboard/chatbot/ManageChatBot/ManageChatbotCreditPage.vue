...
<template>
    <q-card flat bordered>
        <q-layout container style="min-height: 800px;" view="hHh lpR fFf">
            ...
            <q-drawer class="flex column justify-between q-pa-md" show-if-above v-model="showCreditDrawer" side="left" bordered>
                <q-list dense>
                    <template v-for="section in creditSections" :key="section.id">
                        ...
                        <q-item class="q-ma-sm rounded-borders" @click="manageCreditSection = section.id"
                                    :active="manageCreditSection === section.id" active-class="bg-blue-1 text-blue" clickable v-ripple v-else>
                ...
                <q-page class="q-pa-md">
                    <q-tab-panels v-model="manageCreditSection">
                        <q-tab-panel name="chatbot_credit_transactions">
                            ...
                            ]" :rows="chatbotCreditSubscriptionTransactions" :loading="chatbotCreditSubscriptionTransactionsIsLoading">
                                <template v-slot:body-cell-is_paid="props">
                                    <q-td>
                                        <q-badge :label="props.row.is_paid ? 'پرداخت شده' : 'پرداخت نشده'" :color="props.row.is_paid ? 'teal-1' : 'red-1'" :class="props.row.is_paid ? 'text-teal' : 'text-red'"></q-badge>
                                    </q-td>
                                </template>
                                <template v-slot:body-cell-is_extra="props">
                                    <q-td>
                                        <q-badge :label="props.row.is_extra ? 'بله' : 'خیر'" :color="props.row.is_extra ? 'teal-1' : 'red-1'" :class="props.row.is_extra ? 'text-teal' : 'text-red'"></q-badge>
                                    </q-td>
                                </template>
                                <template v-slot:body-cell-actions="props">
                                    <q-td>
                                        <q-btn :label="!props.row.is_paid ? 'پرداخت' : 'پرداخت شده'" :color="props.row.is_paid ? 'grey' : 'blue'" size="sm" :disable="props.row.is_paid"></q-btn>
                                    </q-td>
                                </template>
                            </q-table>
                        </q-tab-panel>
                        <q-tab-panel name="chatbot_credit_buy_subscription">
                            ...
                            <template v-else>
                                <div v-if="!chatbotCreditSubscriptionPlans || chatbotCreditSubscriptionPlans.length < 1">
                                    پلنی برای نمایش وجود ندارد...
                                </div>
                                <div v-else  class="row q-col-gutter-md" >
                                    <div v-for="plan in chatbotCreditSubscriptionPlans.filter(x => x.is_active)" :key="plan.uuid" class="col-12 col-md-4">
                                        <q-card flat bordered class="bg-grey-1" >
                                            ...
                                            <q-card-section>
                                                ...
                                                <div v-else class="text-h6">
                                                    {{plan.original_price}} <TomanSymbol :size="24"></TomanSymbol> / در ماه
                                                </div>
                                            </q-card-section>
                                            <q-card-section style="border-top: 1px solid #e2e2e2;">
                                                <q-list>
                                                    <q-item>
                                                        <q-item-section side><q-icon name="done"></q-icon></q-item-section>
                                                        <q-item-section>{{ plan.extend_days }} روزه</q-item-section>
                                                    </q-item>
                                                    <q-item>
                                                        <q-item-section side><q-icon name="done"></q-icon></q-item-section>
                                                        <q-item-section>{{ plan.extend_token_count }} توکن</q-item-section>
                                                    </q-item>
                                                    <q-item>
                                                        <q-item-section side><q-icon name="done"></q-icon></q-item-section>
                                            ...
                                            <q-card-section style="border-top: 1px solid #e2e2e2;">
                                                <q-btn class="full-width" unelevated :label="plan.original_price ? 'خرید اشتراک' : 'فعال سازی اشتراک'" color="teal-1" text-color="teal" @click="async () => {
                                                    await buyPlan({
                                                        planId: plan.uuid,
                                                    });
