...
const {
  ...
  isFetching,
  refetch: refetchList,
} = shopTablesService.queries.getAll(route.params.storeId as string);
const { mutateAsync: createShopTable } = shopTablesService.mutations.create(
...
const handleClickUrlCell = (url: string) => {
  navigator.clipboard.writeText(url);
  notify.success('به بریده دان انتقال یافت');
};
/********************** */
...
<template>
  <q-dialog v-model="showCreateDialog">
    <q-card>
      <q-card-section> ایجاد میز جدید </q-card-section>
      <q-card-section>
        <q-form @submit.prevent="handleCreate">
          <q-input
            class="q-my-md"
            type="text"
            v-model="createTableModel.title"
            :rules="[
              (val) =>
                (val && val.length > 0) ||
                $t('general.fields.requiredStringField'),
            ]"
            label="عنوان میز"></q-input>
  ...
  <q-dialog v-model="showUpdateDialog">
    <q-card>
      <q-card-section> ویرایش میز </q-card-section>
      <q-card-section>
        <q-form @submit.prevent="handleUpdate">
          <q-input
            class="q-my-md"
            type="text"
            v-model="updateTableModel.title"
            :rules="[
              (val) =>
                (val && val.length > 0) ||
                $t('general.fields.requiredStringField'),
    ...
    <q-table
      ...
      <template v-slot:body-cell-url="props">
        <q-td :props="props">
          <div
            style="z-index: 2"
            @click="handleClickUrlCell(props.row.url as string)">
        ...
        <q-td :props="props">
          <div>
            <q-btn
              ...
              flat
              label="ویرایش"
              @click="handleClickUpdateButton(props.row)"></q-btn>
            <q-btn
              dense
              size="sm"
              label="حذف"
