        ...
        <q-card-section v-if="group.items.length">
          <div class="row q-col-gutter-md">
            <div v-for="item in group.items" :key="item.id"  class="col-12 col-md-4">
              <q-card :class="item.is_active ? 'q-hoverable cursor-pointer' : 'bg-grey'" :v-ripple="item.is_active" flat bordered @click="() => item.is_active ? router.push({ name: 'ManageStorePaymentMethodEditPage', params: { paymentMethodId: item.id } }) : ''">
